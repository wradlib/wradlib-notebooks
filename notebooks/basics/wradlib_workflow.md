...
utm = pyproj.CRS.from_epsg(32632)
print(utm)
...
```{seealso}
Get more info in the section [Converting reflectivity to rainfall](wradlib_get_rainfall) and in the library reference sections {mod}`wradlib.zr` and {mod}`wradlib.trafo`. Here you will learn about the effects of the Z-R parameters ``a`` and ``b``.
```
